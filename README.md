...
# 或从 GitHub Releases 下载（arm64，含 remindkit + fetch-remindkit 子进程 + skill）
mkdir -p ~/.local/bin \
...
# 或使用安装脚本（默认装到 ~/.local/bin）——装完自动注册 skill，无需手动操作
./Scripts/install.sh

# 手动解压安装（非 install.sh）后，跑一次这条让 AI agent 学会用：
...
### 让 AI agent 自动发现 remindkit

remindkit 内置 agent skill（[Agent Skills 标准](https://agentskills.io/specification)），安装后自动注册到 `~/.agents/skills/remindkit/`：

- **install.sh** 装完自动执行 `remindkit install-skill --agents --force`，无需手动操作；跳过用 `REMINDKIT_SKIP_SKILL=1`
- **手动解压 / Homebrew** 安装后运行一次 `remindkit install-skill --agents`
- **Claude Code** 用户额外运行 `remindkit install-skill --claude`
- 重跑安装会以 `--force` 刷新 skill 到最新版（不会因目标已存在而残留旧版）

> 机制：agent 启动时扫描 `~/.agents/skills/`（pi、Codex 等），通过 SKILL.md 的 description 匹配意图后按需加载完整指令。skill 文件在 `~/.local/bin/.agents/skills/remindkit/`（随二进制分发），`install-skill` 负责复制到 agent 目录。

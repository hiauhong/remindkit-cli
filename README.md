...
| 类型 | 读取 | 创建 | 说明 |
|------|:---:|:---:|------|
| 普通列表 | ✅ | 增：✅ `create-list` / 改：✅ `update-list`（改名/图标/12 色板颜色）/ 删：✅ `delete-list` | |
| 智能列表 | ✅（`dump.smartLists`）| ❌ 创建；✅ `delete-list` 可删 | 虚拟视图；含 filterData |
| 分组（文件夹）| ✅ | ❌ | `list --groups` |
...
remindkit complete <id>            # 完成（--reopen 重开）
remindkit delete <id>              # 删除（软删除 → 最近删除）
remindkit move <id> --to 测试列表2
remindkit create-list "新列表"      # 新建列表
